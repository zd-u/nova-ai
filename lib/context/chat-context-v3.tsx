...
import { ActiveMessageScheduler } from '@/lib/proactive/message-scheduler';
import { enhanceTextWithEmotion } from '@/lib/voice/speech-service';
import { getPreferenceLearner } from '@/lib/learning/preference-learner';
import {
  getPersonalizationContext,
  generatePersonalizedSystemPrompt,
  enhanceReplyWithPersonalization,
} from '@/lib/ai/personalized-reply';
...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  const learnerRef = React.useRef<ReturnType<typeof getPreferenceLearner> | null>(null);
  ...
  // 初始化偏好学习器
  const initializeLearner = useCallback(async () => {
    try {
      learnerRef.current = getPreferenceLearner();
      await learnerRef.current.initialize();
    } catch (error) {
    ...
    async (message: string) => {
      try {
        ...
        if (learnerRef.current) {
          await learnerRef.current.learnFromMessage(message, state.emotionalState);
        }
        ...
        // 获取个性化上下文
        const personalizationContext = learnerRef.current
          ? getPersonalizationContext(learnerRef.current)
          : {
              userPreferences: '',
              favoriteTopics: [],
        ...
        let reply = data.result?.reply || '我有点不知道说什么呢...';

        // 增强文本的情感表达
        let enhancedReply = enhanceTextWithEmotion(reply, state.emotionalState);

        // 增强个性化程度
        enhancedReply = enhanceReplyWithPersonalization(
          enhancedReply,
          personalizationContext
        );
